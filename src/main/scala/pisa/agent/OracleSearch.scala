...
import pisa.agent.{ProofText, logProbs}
...
class Oracle(use_proof: Boolean = false, use_conjecture: Boolean = false, use_state_first: Boolean = false,
                 ...
                 engine_address : String = "", dump_path : String = "") {
  implicit val formats : DefaultFormats = DefaultFormats
  implicit val ec: ExecutionContext = ExecutionContext.global

  // Load an oracle agent from the training source
  val path_to_seq2seq : String = "/home/ywu/PISA/data/seq2seq/seq2seq_with_proof_and_state"
  private val src_source = Source.fromFile(path_to_seq2seq + "/train.src")
  val src_list : List[String] = src_source.getLines().toList.map(_.trim)
  src_source.close()
  private val tgt_source = Source.fromFile(path_to_seq2seq + "/train.tgt")
  val tgt_list : List[String] = tgt_source.getLines().toList.map(_.trim)
  tgt_source.close()
  var oracle_map : Map[String, String] = (src_list zip tgt_list).toMap
  ...
  def get_request_string(proof_string: String, state_string: String, initial_step : Boolean = false) : String = {
    if (use_conjecture) {
      ...
      s"""curl ${used_engine_id}
         |-H 'Content-Type: application/json'
         |-H 'Authorization: Bearer <OPENAI_TOKEN>'
         |-d '{"prompt": "[IS] PROOF""".stripMargin + " " + proof_string +
        " <PS_SEP> [IS] GOAL " + state_string + " " +
        s"""PROOFSTEP", "best_of": $search_width, "temperature": $temperature,
           |"max_tokens": $max_tokens, "logprobs": 1, "n": $search_width}'""".stripMargin
    } else {
      s"""curl ${used_engine_id}
...
//        val candidate_logprobs : List[Double] = process_json_value_to_logprobs(parsed_value)
        ...
        val query_string = s"Proof: $proof_string <PS_SEP> State: $state_string"
//        println(s"Query string: $query_string")
//        if (!oracle_map.contains(query_string)) {
//          println(s"Query string: $query_string")
//        }
//        println(oracle_map(query_string))
        val candidate_commands : List[String] = List(oracle_map(query_string))
        val candidate_logprobs : List[Double] = List(0.0)
            ...
            try {
              ...
              if (use_conjecture && proof_command.startsWith("have")) {
                ...
                child_toplevel_state_proof_level_listbuffer.prepend(
                  Tuple4(
                    pisaos.step("sorry", child_toplevel, 10000),
...
//    val replacements = Map("/home/ywu/afp-2021-02-11/".r -> "/root/afp-2021-02-11/")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val search_agent = new Oracle(use_proof=use_proof, use_conjecture=use_conj,
